...
NBAteams <- within(NBAteams,  Team <- paste(location, nickname, sep=" "))
...
Teams1 <- NBATeamGameLogs_Full_Data[c('team_id','teamname','teamslug','Type','Win')]
NBAteams2 <- NBAteams[c('team_id','Team')]
AVG_Win_Per_Season_NBA <- sqldf("SELECT A.team_id AS TeamId,teamname AS TeamName, B.Team, teamslug AS Abbr, sum(Win)/2 AS Wins
                                FROM Teams1 A
                                LEFT JOIN NBAteams2 B
                                    ON A.team_id = B.team_id
                                WHERE Type = 'RegularSeason'
                                GROUP BY A.team_id,A.teamname")
...
NBA_Salary <- url %>% read_html() %>%
  html_table(fill = TRUE)
NBA_Salary <- NBA_Salary[[1]]
NBA_Salary <- NBA_Salary[c('Team','Total Cap')]
names(NBA_Salary)[1]<-"Team"
names(NBA_Salary)[2]<-"TotalCap2016_2017"
NBA_Salary$TotalCap2016_2017 <- gsub(",","",NBA_Salary$TotalCap2016_2017) 
NBA_Salary$TotalCap2016_2017 <- as.numeric(gsub("\\$", "", NBA_Salary$TotalCap2016_2017))


Wins_vs_Salary_NBA <- sqldf("SELECT A.TeamId, TeamName, Abbr, Wins, TotalCap2016_2017 AS TeamSalary
                            FROM AVG_Win_Per_Season_NBA A
                            INNER JOIN NBA_Salary B
                            ON A.Team = B.Team")
...
NBA_Model_Accuracy <- (441+132)/(441+132+150+63)
NBA_Model_Accuracy

#Baseline 
NBA_Baseline <- (441+63)/(441+63+150+132)
NBA_Baseline
...
plot(perf)
title('ROC Curve for NBA Tree')
text(0.6, 0.4, labels=("AUC = 0.71"), col="red")
dev.off()

# The model could differentiate between blowouts and non-blowouts pretty well 
# with an out-of-sample AUC of 0.71.

NBA_AUC <- as.numeric(performance(pred, "auc")@y.values)
NBA_AUC
...
NBA_Random_Forest_Model_Accuracy <- (465+143)/(465+143+138+39)
NBA_Random_Forest_Model_Accuracy
