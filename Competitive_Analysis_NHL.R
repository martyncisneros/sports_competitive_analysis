...
NHLteams <- within(NHLteams,  Team <- paste(location, nickname, sep=" "))
...
Teams4 <- NHLTeamGameLogs_Full_Data[c('team_id','teamname','teamslug','Type','Win')]
NHLteams2 <- NHLteams[c('team_id','Team')]
AVG_Win_Per_Season_NHL <- sqldf("SELECT A.team_id AS TeamId,teamname AS TeamName, B.Team, teamslug AS Abbr, sum(Win)/2 AS Wins
                                FROM Teams4 A
                                LEFT JOIN NHLteams2 B
...
url <- "http://www.spotrac.com/nhl/cap/"
NHL_Salary <- url %>% read_html() %>%
  html_table(fill = TRUE)
NHL_Salary <- NHL_Salary[[1]]
NHL_Salary <- NHL_Salary[c('Team','Est. Total Cap')]
names(NHL_Salary)[2]<-"TotalCap2016_2017"
NHL_Salary$TotalCap2016_2017 <- gsub(",","",NHL_Salary$TotalCap2016_2017) 
NHL_Salary$TotalCap2016_2017 <- as.numeric(gsub("\\$", "", NHL_Salary$TotalCap2016_2017))
NHL_Salary$Team[NHL_Salary$Team == "St Louis Blues"] <- "St. Louis Blues"


Wins_vs_Salary_NHL <- sqldf("SELECT A.TeamId, A.Team, TeamName, Abbr, Wins, TotalCap2016_2017 AS TeamSalary
                            FROM AVG_Win_Per_Season_NHL A
                            INNER JOIN NHL_Salary B
                            ON A.Team = B.Team")
...
NHL_Model_Accuracy <- (229+190)/(229+190+180+190)
NHL_Model_Accuracy

#Baseline 
NHL_Baseline <- (229+190)/(229+190+180+190)
NHL_Baseline
...
plot(perf)
title('ROC Curve for NHL Tree')
text(0.6, 0.4, labels=("AUC = 0.53"), col="red")
dev.off()

NHL_AUC <- as.numeric(performance(pred, "auc")@y.values)
...
NHL_Random_Forest_Model_Accuracy <- (301+176)/(301+176+194+118)
NHL_Random_Forest_Model_Accuracy
