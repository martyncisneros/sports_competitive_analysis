...
Teams <- NFLTeamGameLogs_Full_Data[c('team_id','teamname','teamslug','Type','Win')]
AVG_Win_Per_Season_NFL <- sqldf("SELECT team_id AS TeamId,teamname AS TeamName, teamslug AS Abbr, sum(Win)/2 AS Wins
                                FROM Teams
...
colnames(NFL_Players)[1] <- 'player_id' 
colnames(NFL_Rosters17)[1] <- 'player_id' 

NFL_Players_DF <- NFL_Players[which(NFL_Players$active == 'TRUE'),]
NFL_Players_DF <- NFL_Players_DF[c('name','salary','humanized_salary','salary_currency','league_id','team_id','player_id')]
Team_Salary_NFL <- sqldf("SELECT A.team_id AS TeamId, sum(A.salary) AS TeamSalary, count(*) AS NumberofPlayers
                         FROM NFL_Players A
                         INNER JOIN NFL_Rosters17 B
                            ON A.player_id = B.player_id AND A.team_id = B.team_id
                         GROUP BY A.team_id")
# Merge Dataframes

Wins_vs_Salary_NFL <- sqldf("SELECT A.TeamId, TeamName, Abbr, Wins, TeamSalary, NumberofPlayers
                            FROM AVG_Win_Per_Season_NFL A
                            INNER JOIN Team_Salary_NFL B
...
NFL_CloseGame_Log = glm(IsCloseGame3 ~ DeltaFGA+Delta4DownSucceeded
                      +DeltaFumbles+DeltaPassing1Downs+DeltaPassingNetYards
                      +DeltaPenaltyYards+DeltaRushing1Downs+DeltaRushingNetYards
                      +Delta3DownSucceeded+DeltaTOP_secs
                      +DeltaTotalFirstDowns+ DeltaTeamSalary
                      , data = Train, family=binomial)
summary(NFL_CloseGame_Log)

# Predictions on the test set
predictTest = predict(NFL_CloseGame_Log, type="response", newdata=Test)
...
# However; the model not so great at differentiating between close games and non-close games pretty well 
# with an out-of-sample AUC of 0.60.


############## Feature Importance #############

# estimate variable importance
importance <- varImp(NFL_CloseGame_Forest, scale=FALSE)
