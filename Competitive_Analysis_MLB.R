...
MLBteams <- within(MLBteams,  Team <- paste(location, nickname, sep=" "))
...
Teams2 <- MLBTeamGameLogs_Full_Data_imputed[c('team_id','teamname','teamslug','Type','Win')]
MLBteams2 <- MLBteams[c('team_id','Team')]
AVG_Win_Per_Season_MLB <- sqldf("SELECT A.team_id AS TeamId,teamname AS TeamName, B.Team, teamslug AS Abbr, sum(Win)/2 AS Wins
                                FROM Teams2 A
                                LEFT JOIN MLBteams2 B
...
url <- "http://www.spotrac.com/mlb/payroll/2016/"
MLB_Salary <- url %>% read_html() %>%
  html_table(fill = TRUE)
MLB_Salary <- MLB_Salary[[1]]
MLB_Salary <- MLB_Salary[c('Team','Total Payroll')]
names(MLB_Salary)[2]<-"TotalCap2016_2017"
MLB_Salary$TotalCap2016_2017 <- gsub(",","",MLB_Salary$TotalCap2016_2017) 
MLB_Salary$TotalCap2016_2017 <- as.numeric(gsub("\\$", "", MLB_Salary$TotalCap2016_2017))
MLB_Salary <- MLB_Salary[MLB_Salary$Team != "League Average", ]
MLB_Salary$Team[MLB_Salary$Team == "Los Angeles Angels of Anaheim"] <- "Los Angeles Angels"


Wins_vs_Salary_MLB <- sqldf("SELECT A.TeamId, A.Team, TeamName, Abbr, Wins, TotalCap2016_2017 AS TeamSalary
                            FROM AVG_Win_Per_Season_MLB A
                            INNER JOIN MLB_Salary B
                            ON A.Team = B.Team")
...
# Split Data
set.seed(3000)
spl = sample.split(MLBGameData_Models$IsBlowOut3, SplitRatio = 0.7)
Train = subset(MLBGameData_Models, spl==TRUE)
Test = subset(MLBGameData_Models, spl==FALSE)
...
PredictCART = predict(MLB_Blow_Outs_Tree, newdata = Test, type = "class")
table(Test$IsBlowOut3, PredictCART)

#Measure accuracy with confusion matrix
MLB_Model_Accuracy <- (892+315)/(892+315+298+114)
MLB_Model_Accuracy

#Baseline 
MLB_Baseline <- (892+114)/(892+315+298+114)
MLB_Baseline

PredictROC = predict(MLB_Blow_Outs_Tree, newdata = Test)
...
plot(perf)
title('ROC Curve for MLB Tree')
text(0.6, 0.4, labels=("AUC = 0.75"), col="red")
dev.off()

MLB_AUC <- as.numeric(performance(pred, "auc")@y.values)
...
MLB_Random_Forest_Model_Accuracy <- (876+346)/(876+346+267+130)
MLB_Random_Forest_Model_Accuracy
